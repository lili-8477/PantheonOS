...
class ContextCompressor:
    ...
    def should_compress(
        ...
        if self.config.threshold == 0:
            ...
            valid_count = sum(1 for m in uncompressed_messages if m.get("role") != "system")
        ...
        if max_tokens == 0:
            ...
            if model and get_model_info:
                try:
                    info = get_model_info(model)
                    max_tokens = (info.get("max_input_tokens") or 0) + (info.get("max_output_tokens") or 0)
                except Exception:
                    pass
    ...
    async def compress(
        ...
        details_path = f"{compression_dir}/compression_{compression_id}.json" if compression_dir else None
        ...
        if result.viewed_files or result.edited_files:
            viewed_list = '\\n  - '.join(f'`{f}`' for f in result.viewed_files[:20]) if result.viewed_files else 'None'
            edited_list = '\\n  - '.join(f'`{f}`' for f in result.edited_files[:20]) if result.edited_files else 'None'
