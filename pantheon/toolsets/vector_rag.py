...
class VectorRAGToolSet(ToolSet):
    ...
    async def query_vector_db(
        self, query: str, top_k: int = 3, source: str | None = None
    ) -> list:
        ...
        """
        logger.info(
            f"[cyan]🔍 Querying vector database: {query[:50]}{'...' if len(query) > 50 else ''}[/cyan]"
        )
        if source:
    ...
    async def get_vector_db_info(self) -> dict:
        ...
        if metadata:
            logger.info(
                f"[green]✅ Database info retrieved: {metadata.get('description', 'No description')}[/green]"
            )
        else:
    ...
    async def insert_vector_db(self, text: str, metadata: dict | None = None):
        ...
        logger.info(
            f"[cyan]📝 Inserting document: {text[:50]}{'...' if len(text) > 50 else ''}[/cyan]"
        )
