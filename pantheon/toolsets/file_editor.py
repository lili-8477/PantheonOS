...
class FileEditorToolSet(ToolSet):
    ...
    def _print_diff_display(
        ...
        removals_text = (
            f"{removals} removal{'s' if removals != 1 else ''}" if removals > 0 else ""
        )
        ...
        for tag, i1, i2, j1, j2 in matcher.get_opcodes():
            if tag == "equal":
                ...
                for idx in range(i1, i2):
                    line_content = (
                        original_lines[idx] if idx < len(original_lines) else ""
                    )
                    logger.info(
                        f"       [on red]{old_line_num:3d} -              {line_content}[/on red]"
                    )
                    old_line_num += 1
                ...
                for idx in range(j1, j2):
                    ...
                    logger.info(
                        f"       [on green]{new_line_num:3d} +              {line_content}[/on green]"
                    )
                    new_line_num += 1
    ...
    async def read_file(
        ...
        try:
            ...
            if show_line_numbers:
                ...
                logger.info("")
                logger.info(
                    f"╭─ [bold]File content:[/bold] {display_path} "
                    + "─" * (65 - 15 - len(display_path))
                    + "╮"
                )
                ...
                for i, line in enumerate(selected_lines, start=start + 1):
                    ...
                    logger.info(
                        f"   [dim]{line_no}[/dim] │ {line_content}" + " " * padding
                    )
    ...
    async def write_file(self, file_path: str, content: str) -> dict:
        ...
        try:
            ...
            logger.info(
                f"[green]File {action_word} successfully - {lines_count} lines written[/green]"
            )
    ...
    async def edit_file(
        ...
        try:
            ...
            # Show success message
            logger.info(
                f"[green]File edited successfully - {replaced_count} replacement(s) made[/green]"
            )
    ...
    async def insert_at_line(
        self, file_path: str, line_number: int, content: str, position: str = "after"
    ) -> dict:
        """
    ...
    async def delete_lines(
        ...
        try:
            ...
            return {
                ...
                "new_total_lines": len(lines),
                "deleted_preview": "".join(deleted_content[:5]) + "..."
                if len(deleted_content) > 5
                else "".join(deleted_content),
            }
    ...
    async def list_files(
        self, directory: str = ".", pattern: str | None = None, recursive: bool = False
    ) -> dict:
        """
