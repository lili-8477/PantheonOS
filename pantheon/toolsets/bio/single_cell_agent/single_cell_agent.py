...
class SingleCellAgentToolSet(ToolSet):
    ...
    def _initialize_agent(self):
        ...
        try:
            from .single_cell_agent_deps import (
    ...
    async def single_cell_agent(
        ...
        try:
            ...
            if (
                hasattr(self, "_agent_ready")
                and self._agent_ready
                and not hasattr(self, "_first_use_logged")
            ):
            ...
            if analysis_type == "comprehensive":
                results = await self._comprehensive_analysis(
                    dataset, research_question, visualize, output_dir
                )
            elif analysis_type == "annotation":
                results = await self._annotation_analysis(
                    dataset, visualize, output_dir
                )
            elif analysis_type == "trajectory":
                results = await self._trajectory_analysis(
                    dataset, visualize, output_dir
                )
            elif analysis_type == "differential":
                results = await self._differential_analysis(
                    dataset, visualize, output_dir
                )
                ...
                results = await self._custom_analysis(
                    dataset, research_question, visualize, output_dir
                )
            else:
                results = {
    ...
    async def _comprehensive_analysis(
        self, dataset: str, research_question: str, visualize: bool, output_dir: Path
    ) -> Dict[str, Any]:
        ...
        with Progress(
            ...
            progress.update(
                task, advance=1, description="🌊 Step 2: Trajectory Analysis..."
            )
            ...
            progress.update(
                task, advance=1, description="📈 Step 3: Differential Expression..."
            )
            ...
            progress.update(
                task, advance=1, description="🛤️ Step 4: Pathway Analysis..."
            )
            if results.get("differential_expression", {}).get("top_genes"):
                ...
                if "get_pathway_enrichment_for_genes" in func2info:
                    pathway_result = func2info["get_pathway_enrichment_for_genes"][0](
                        ",".join(genes)
                    )
            ...
            if visualize:
                progress.update(
                    task, advance=1, description="🎨 Step 5: Creating Visualizations..."
                )
            ...
            # Step 6: Integration
            progress.update(
                task, advance=1, description="🔄 Step 6: Integrating Results..."
            )
            results["integration"] = self._integrate_results(results)

            # Step 7: Interpretation
            progress.update(
                task, advance=1, description="🧠 Step 7: Biological Interpretation..."
            )
    ...
    async def _annotation_analysis(
        self, dataset: str, visualize: bool, output_dir: Path
    ) -> Dict[str, Any]:
    ...
    async def _trajectory_analysis(
        self, dataset: str, visualize: bool, output_dir: Path
    ) -> Dict[str, Any]:
    ...
    async def _differential_analysis(
        self, dataset: str, visualize: bool, output_dir: Path
    ) -> Dict[str, Any]:
        ...
        if "get_differential_expression_for_groups" in func2info:
            ...
            results["differential_expression"] = func2info[
                "get_differential_expression_for_groups"
            ][0](dataset, "cell_type", method="DCT")

        # Pathway enrichment for DE genes
        if results.get("differential_expression", {}).get("top_genes"):
            genes = results["differential_expression"]["top_genes"]
            if "get_pathway_enrichment_for_genes" in func2info:
                results["pathway_enrichment"] = func2info[
                    "get_pathway_enrichment_for_genes"
                ][0](",".join(genes))

        return results

    async def _visualization_analysis(
        self, dataset: str, output_dir: Path
    ) -> Dict[str, Any]:
        ...
        if "get_embedding_visualization_for_dataset" in func2info:
            ...
            results["embedding_plot"] = func2info[
                "get_embedding_visualization_for_dataset"
            ][0](dataset, color_by="cell_type", basis="umap", save_path=save_path)
        ...
        if "get_gene_expression_visualization_for_dataset" in func2info:
            ...
            results["gene_expression_plot"] = func2info[
                "get_gene_expression_visualization_for_dataset"
            ][0](dataset, genes=marker_genes, plot_type="violin", save_path=save_path)
    ...
    async def _communication_analysis(
        self, dataset: str, output_dir: Path
    ) -> Dict[str, Any]:
    ...
    async def _drug_response_analysis(
        self, dataset: str, output_dir: Path
    ) -> Dict[str, Any]:
    ...
    def _integrate_results(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "cell_annotation" in results and results["cell_annotation"].get("success"):
            integration["total_cells"] = results["cell_annotation"].get(
                "total_cells", 0
            )
            integration["cell_types_identified"] = results["cell_annotation"].get(
                "unique_cell_types", 0
            )

        if "differential_expression" in results and results[
            "differential_expression"
        ].get("success"):
            integration["significant_genes"] = results["differential_expression"].get(
                "significant_genes", 0
            )

        if "pathway_enrichment" in results and results["pathway_enrichment"].get(
            "success"
        ):
            integration["enriched_pathways"] = results["pathway_enrichment"].get(
    ...
    def _interpret_results(
        self, results: Dict[str, Any], research_question: str = None
    ) -> str:
        ...
        if "cell_annotation" in results and results["cell_annotation"].get("success"):
            ...
            interpretation.append(
                f"**Cell Composition**: Identified {anno.get('unique_cell_types', 0)} distinct cell types"
            )
        ...
        if "trajectory" in results and results["trajectory"].get("success"):
            ...
            interpretation.append(
                f"**Developmental Dynamics**: Trajectory analysis reveals"
            )
            interpretation.append(
                f"{traj.get('trajectory_branches', 1)} developmental branches.\n"
            )
        ...
        if "differential_expression" in results and results[
            ...
            interpretation.append(
                f"**Gene Expression**: Found {de.get('significant_genes', 0)} differentially"
            )
        ...
        if "pathway_enrichment" in results and results["pathway_enrichment"].get(
            ...
            interpretation.append(
                f"**Functional Enrichment**: {pathway.get('significant_pathways', 0)} pathways"
            )
            interpretation.append(
                f"significantly enriched, suggesting active biological processes.\n"
            )
    ...
    def _display_results(self, results: Dict[str, Any], analysis_type: str):
        ...
        for component, data in results.items():
            ...
            if component not in {
                "integration",
                "interpretation",
            }:  # skip meta sections in table rows
