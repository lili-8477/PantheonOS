...
class ScRNASeqUpstreamToolSet(ScRNASeqBase):
    ...
    async def install_missing_packages(
        ...
        with Progress(
            ...
            for package in packages:
                ...
                try:
                    ...
                    installation_results[package] = {"status": "error", "error": str(e)}
        ...
        successful = [
            pkg
            for pkg, result in installation_results.items()
    ...
    def scan_folder(self, folder_path: str) -> Dict[str, Any]:
        ...
        for file_path in all_files:
            ...
            scan_results["file_types"][ext].append(
    ...
    def _identify_data_patterns(self, folder_path: Path, scan_results: Dict):
        ...
        expression_files = (
            list(folder_path.rglob("*expression*"))
    ...
    def _display_scan_results(self, scan_results: Dict):
        ...
        if scan_results["recommendations"]:
            recommendations_text = "\n".join(
                [f"• {rec}" for rec in scan_results["recommendations"]]
            )
            recommendations_panel = Panel(
                recommendations_text, title="Recommendations", border_style="blue"
            )
            logger.info("", rich=recommendations_panel)

    @tool
    def init(
        self, project_name: str = None, create_structure: bool = True
    ) -> Dict[str, Any]:
...
"""
                ...
                logger.info(
                    f"✅ [green]Project structure created successfully![/green]"
                )
