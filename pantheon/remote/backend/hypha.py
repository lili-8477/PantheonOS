...
class HyphaBackend(RemoteBackend):
    ...
    async def create_worker(self, service_name: str, **kwargs) -> "HyphaRemoteWorker":
        """Create a worker for serving functions"""
        server = await self._get_server()
        return HyphaRemoteWorker(server, service_name, **kwargs)
...
class HyphaService(RemoteService):
    ...
    def service_info(self) -> ServiceInfo:
        ...
        return ServiceInfo(service_id=self._service_id, service_name=self._service_name)
...
class HyphaRemoteWorker(RemoteWorker):
    ...
    def __init__(
        self, server, service_name: str, service_id: Optional[str] = None, **kwargs
    ):
        self.server = server
    ...
    async def run(self):
        ...
        for func_name, func in self.functions.items():
            if asyncio.iscoroutinefunction(func):
                ...
                service_interface[func_name] = async_wrapper
